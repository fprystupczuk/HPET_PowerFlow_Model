...
V1_points = 200
V1 = np.linspace(1.0,0.6,V1_points) # Linear sweep of Q2
FractionPowerHT = np.array([0.3, 0.25, 0.2, 0.15, 0.1, 0.05]) # FractionPowerHT = np.array([0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3])

# Variables to store results
V_LV_model = np.zeros((V1.size,FractionPowerHT.size))
...
for j in range(FractionPowerHT.size):
    ...
    for i in range(V1_points):
        
        # Assign load in function of the load sweep through all the values of Q2
        DSSText.Command = 'Edit Vsource.source pu=' + str(V1[i])
...
#            DSSCircuit.Loads.kvar = Qc1[phase] # In this case Qc1 is not cero, to compensate primary side Q
        ...
        DSSMonitors.Name = 'Trafo_sec-VI'
        for phase in range(3):
            V_LV_model[i,j] = DSSMonitors.Channel(phase * 2 + 1)[0]


# FIGURES
        
sns.set_palette("tab10",FractionPowerHT.size) # See colormaps on https://matplotlib.org/tutorials/colors/colormaps.html
figure_size = (10, 9)
font_size = 20
...
for j in range(FractionPowerHT.size):
    ax.plot(V1,V_LV_model[:,j]/Vwin_nom[1], label=r'$\alpha$ = ' + str(FractionPowerHT[j]))
ax.plot(V1,V1, '--',color='silver', label='No PEC')
ax.set_xlim(V1[0]+0.02, V1[-1]-0.02)
plt.grid(True)
ax.set(xlabel = r'$V_{MV}\mathrm{\ [pu]}$', ylabel= r'$V_{LV}\mathrm{\ [pu]}$')
ax.legend()
#plt.title(r'$Q_1$ vs. $Q_2$ @ Noloadloss=0.08125, Loadloss=0.875')
fig.savefig('./test_HTlines_VLinearSweep_Delta_Graph/VLVvsVMV_Noloadloss0.08125_Loadloss0.875.pdf')
#plt.title(r'$Q_1$ vs. $Q_2$ @ Noloadloss=0, Loadloss=0')
#fig.savefig('./test_HTlines_VLinearSweep_Graph/Q1vsQ2_Noloadloss0.0_Loadloss0.0.pdf')
